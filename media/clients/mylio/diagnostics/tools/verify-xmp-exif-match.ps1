# Verify that XMP sidecars match EXIF timestamps for 2006 July files
# Compares XMP:DateTimeOriginal with EXIF:DateTimeOriginal
...
Write-Console "=== Verifying XMP and EXIF timestamps match ===`n" -ForegroundColor Cyan
...
foreach ($file in $files) {
    ...
    if ($file -match '^(\d{4})-(\d{2})') {
        ...
        # Get EXIF date from the actual file (format varies by file type)
        $exifDate = & $exiftoolPath -s -s -s -EXIF:DateTimeOriginal $fullPath 2>$null
        ...
        # Videos may not have EXIF DateTimeOriginal - check XMP only for them
        if ($ext -match '\.(mpg|mpeg|mp4|avi|mov)$') {
            # For videos, just verify XMP date is correct (on July 2, 2006)
            if ($xmpDate -match '^2006:07:02') {
                $matched++
            } else {
                Write-Console "  VIDEO XMP WRONG DATE: $file" -ForegroundColor Yellow
        ...
        # For photos, compare XMP and EXIF
        if (-not $exifDate) {
            Write-Console "  NO EXIF DATE: $file (XMP: $xmpDate)" -ForegroundColor Yellow
            $errors++
            continue
        }

        # Normalize dates for comparison (remove milliseconds from XMP)
        ...
        if ($xmpDateNorm -eq $exifDateNorm) {
            ...
            Write-Console "  MISMATCH: $file" -ForegroundColor Yellow
            Write-Console "    XMP:  $xmpDate" -ForegroundColor Gray
...
Write-Console "Matched: $matched" -ForegroundColor Green
Write-Console "Mismatched: $mismatched" -ForegroundColor $(if ($mismatched -gt 0) { "Red" } else { "Green" })
Write-Console "Errors: $errors" -ForegroundColor $(if ($errors -gt 0) { "Red" } else { "Green" })

if ($mismatched -gt 0) {
    Write-Console ""
    Write-Console "=== Mismatched Files ===" -ForegroundColor Yellow
