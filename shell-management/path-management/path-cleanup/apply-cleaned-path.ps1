...
Write-Console "  Apply Cleaned PATH" -ForegroundColor Cyan
Write-Console "========================================`n" -ForegroundColor Cyan

# Check if running as administrator
$isAdmin = ([Security.Principal.WindowsPrincipal] [Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)
if (-not $isAdmin) {
    Write-Console "WARNING: Not running as administrator." -ForegroundColor Yellow
    Write-Console "This script will update USER PATH only (not SYSTEM PATH)." -ForegroundColor Yellow
    Write-Console ""
}
...
# Confirm
Write-Console "`nThis will update your USER PATH environment variable." -ForegroundColor Yellow
Write-Console "The change will take effect in NEW terminal windows." -ForegroundColor Yellow
$confirm = Read-Host "`nApply cleaned PATH? (yes/no)"

if ($confirm -ne "yes") {
    Write-Console "`nOperation cancelled." -ForegroundColor Red
    exit 0
}
